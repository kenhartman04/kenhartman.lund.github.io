...
// Initialize the map
var map = L.map("map").setView([55.70466, 13.191007], 13);
...
function loadPumpLayer() {
  var pumpLayer = new L.GeoJSON.AJAX(
    "http://geoserver.gis.lu.se/geoserver/LundBike/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=LundBike:BikePumps&maxFeatures=50&outputFormat=application%2Fjson",
    {
...
function calculateAndDisplayRoute(startPoint, endPoint) {
  ...
  fetch(orsAPI)
    ...
    .then((data) => {
      ...
      var popupContent = "<h2>Route Instructions</h2><ul>";
